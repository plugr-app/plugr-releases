...
**STATUS (2026-07-16 h/i):** Tier 2 (self-serve AI-assist) is fully built + wired (Toolbar "Categorize…" → CategorizeModal → `src/util/taxonomy.js`). Tier 1 (silent gap-reporting) app-side is built too: opt-in (gated on `communityConsent==='allowed'`), fires once per session, sends ONLY `{name, developer}` of uncategorized plugins as one compact list via `api.submitCategoryGaps` → `community:submitGaps` → `community.submitCategoryGaps()`. It **no-ops until Josh configures the gap form** (`GAPS_SUBMIT_URL` + `GAP_FIELDS` in `community.cjs` are blank — TODO markers in place). **ACTION FOR JOSH:** create a "Plugr Category Gaps" Google Form (one long-answer "Gaps" field for the JSON array + a short "App Version" field), link a responses Sheet, paste the formResponse URL + the two `entry.NNN` ids into `community.cjs`. Then it's live. **Gap-review procedure (mirrors §15):** read the gap responses Sheet via the Drive connector → aggregate `{name,developer}` across responses, count duplicates = popularity ranking → categorize the top gaps with knowledge + web research (the 2026-07-16(g) process) → append to `tools/category-seed-supplement.json` → `npm run seed-categories` → ships in the registry next release → all users' undefined plugins fix on next scan. Reuses the SAME curate→review→registry loop, no separate system.
