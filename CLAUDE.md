...
- **Family grouping is conservative about "version markers" (2026-07-12 fix):** `normalizeName()` in duplicates.cjs no longer borrows nameVariants' most-stripped variant (that collapsed Arturia's "Pre V76" and "Pre 1973" — different PRODUCTS — into one "pre" family, cross-flagging OLD). Strips only: trailing " V<single digit>" (Mini V3), trailing bare " V" (ARP 2600 V), trailing bare 1-2 digit number (Neutron 3, Ozone 11 — grouping those IS intended). Keeps V+2-digit (V76) and 3+ digit numbers (1973, 2600) as model numbers.
